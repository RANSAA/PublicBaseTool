...
/**
 ...
 替换原理：
        先新建一个newView替换原来的self.view(默认是tableView),然后将系统默认的tableView添加到新的newView上面。
        tableView的约束条件：left,right.bottom都与superView(newView)的left,right,bottom保持一致；
        tableView的top是以controller.TopLayoutGuide.Bottom为基准进行约束的，这些约束值都可以调整
 其它：
        如果不需要新建newView进行替换默认的self.view只需要重写：setTKEnabledNewView 方法将其置空即可，
        不过重写了之后与newView相关的所有操作都无效了
 附加：
        如果要使tableView的顶部与控制器的最顶部保持一致(即状态栏区域不需要自动填充)时，可以配置contentInsetAdjustmentBehavior属性，
        autoUpdateTableViewTopAnchor = NO 属性与 [self setTKTableViewTopAnchorConstant:offY]方法来处理，
...
/**
 作用：创建一个新的newView替换Controller中的self.view,并将原先的self.tableView添加到newView上面。
 约束方式：self.tableView的左右下与newView的左右下相关连，self.tableView的top方向是基于topLayoutGuide.bottom的。
 功能开关：只需要将该方法重写置空，即可关闭newView相关操作，使之相关联的属性无效化(PS:默认该功能是开启的)
 PS:根据是否开启了该功能决定,talbleView的get函数返回的对象是有区别的：UITableView或者UIView
 */
