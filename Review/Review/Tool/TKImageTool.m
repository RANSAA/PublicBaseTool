 ...
 */
+ (NSArray *)TKStringGetAllFilePathWithBundle:(NSBundle *)bundle dirName:(NSString *)dirName
{
    dirName = dirName == nil?@"/":dirName;
    NSString *dirPath = [bundle resourcePath];
    dirPath = [dirPath stringByAppendingPathComponent:dirName];//指定bundle目录的全路径
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSError *error = nil;
    NSArray *nameAry = [fileManager contentsOfDirectoryAtPath:dirPath error:&error];//获取指定路径中的全部文件的文件名
    //将路径字符进行排序--升序
    __block NSLocale *locale=[[NSLocale alloc] initWithLocaleIdentifier:@"zh_CN"];

    NSArray *sortedArrays = [nameAry sortedArrayUsingComparator:^NSComparisonResult(id  _Nonnull obj1, id  _Nonnull obj2) {
        NSStringCompareOptions comparisonOptions = NSCaseInsensitiveSearch|NSNumericSearch|
        NSWidthInsensitiveSearch|NSForcedOrderingSearch;
        NSRange range = NSMakeRange(0,((NSString *)obj1).length);
        range = NSMakeRange(0,((NSString *)obj2).length);
        locale = nil;

//        return [obj1 compare:obj2 options:comparisonOptions range:range locale:locale];
        return [obj2 compare:obj1 options:comparisonOptions range:range locale:locale];
    }];
    NSMutableArray *filePathAry = @[].mutableCopy;
    for (NSString *name in sortedArrays) {
        [filePathAry addObject:[dirPath stringByAppendingPathComponent:name]];
    }
