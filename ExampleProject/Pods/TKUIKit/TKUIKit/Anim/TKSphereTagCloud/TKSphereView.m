...
@implementation TKSphereView
{
    NSMutableArray *tags;
    NSMutableArray *coordinate;
    TKPoint normalDirection;
    CGPoint last;
...
- (void)setCloudTags:(NSArray *)array
{
    ...
    for (NSInteger i = 0; i < tags.count; i ++) {
        ...
        TKPoint point = TKPointMake(x, y, z);
        NSValue *value = [NSValue value:&point withObjCType:@encode(TKPoint)];
    ...
    normalDirection = TKPointMake(a, b, 0);
    [self timerStart];
}

#pragma mark - set frame of point

- (void)updateFrameOfPoint:(NSInteger)index direction:(TKPoint)direction andAngle:(CGFloat)angle
{
    ...
    TKPoint rPoint = TKPointMakeRotation(point, direction, angle);
    value = [NSValue value:&rPoint withObjCType:@encode(TKPoint)];
...
- (void)setTagOfPoint: (TKPoint)point andIndex:(NSInteger)index
{
...
- (void)handlePanGesture:(UIPanGestureRecognizer *)gesture
{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        TKPoint direction = TKPointMake(last.y - current.y, current.x - last.x, 0);
