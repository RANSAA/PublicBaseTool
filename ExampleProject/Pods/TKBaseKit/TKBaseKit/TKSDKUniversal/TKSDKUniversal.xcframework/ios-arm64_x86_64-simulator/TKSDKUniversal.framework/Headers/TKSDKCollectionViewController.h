...
注意：
        ...
        2.使用setTKEnabledSelfView方法创建了一个新的视图"selfView"用于替换UICollectionViewController中的原始self.view（collectionView）以解决TKNavigationBar导航条问题。
        3.如果重置了setTKEnabledSelfView，那么所有与TKNavigationBar相关的操作将会失效，即不会创建TKNavigationBar。
 

使用场景:
     1.静态collectionView时推荐优先使用该控制器。
     2.直接在UIStoryboard中对静态collectionView进行编辑时。


⚠️⚠️⚠️
使用注意：
     如果使用时出现未知bug，也可以利用TKSDKViewController+UICollectionViewController+UIStoryboard组合方式去使用静态CollectionView
...
/**
 ⚠️⚠️⚠️
 功能：创建一个新的视图"selfView"用于替换controller中的self.view(对应collectionView),并将collectionView添加到selfView上面。
 约束：collectionView的左右下与selfView的左右下关联；self.collectionView.topAnchor是与self.topLayoutGuide.bottom相关联的。
...
/**
 collectionView.leftAnchor距离selfView.leftAnchor的constant值
 */
@property(nonatomic, assign) CGFloat collectionViewLeftAnchorConstant;
/**
 collectionView.rightAnchor距离selfView.rightAnchor的constant值
 */
@property(nonatomic, assign) CGFloat collectionViewRightAnchorConstant;
/**
 collectionView.bottomAnchor距离selfView.bottomAnchor的constant值
 */
